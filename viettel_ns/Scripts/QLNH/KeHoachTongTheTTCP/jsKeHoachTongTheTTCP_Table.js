...
function BangDuLieu_ThemHangMoi(h, hGiaTri, isSameLevel) {
    ...
    Bang_GanGiaTriThatChoO(rowPositionMaxStt, Bang_arrCSMaCot["fGiaTri"], '');
...
function ValidateData() {
    var sMessError = [];
    var sChuongTrinh = "", sBQuanLy = "", sGiaTri = "";
    var Title = 'Lỗi lưu kế hoạch tổng thể TTCP chi tiết';
    if (Bang_nH == 0) {
        sMessError.push("Kế hoạch hiện tại chưa có chương trình, nhiệm vụ chi. Vui lòng thêm chương trình, nhiệm vụ chi.");
    }
    else {
        for (var j = 0; j < Bang_nH; j++) {
            if (!Bang_arrHangDaXoa[j]) {
                ...
                var sTenPhongBan = Bang_LayGiaTri(j, "iID_BQuanLyID");
                if (sTenPhongBan == '' || sTenPhongBan == GUID_EMPTY) {
                    sBQuanLy += Bang_arrGiaTri[j][Bang_arrCSMaCot["sMaThuTu"]] + ", ";
                }
                var fGiaTri = Bang_LayGiaTri(j, "fGiaTri");
                if (fGiaTri == '' || parseFloat(fGiaTri) <= 0) {
                    sGiaTri += Bang_arrGiaTri[j][Bang_arrCSMaCot["sMaThuTu"]] + ", ";
                }
            }
        }
        if (sChuongTrinh != "") {
            sMessError.push('Hãy nhập tên chương trình, nhiệm vụ chi dòng ' + sChuongTrinh.substring(0, sChuongTrinh.length - 2) + '.');
        }
        if (sBQuanLy != "") {
            sMessError.push('Hãy chọn ban quản lý dòng ' + sBQuanLy.substring(0, sBQuanLy.length - 2) + '.');
        }
        if (sGiaTri != "") {
            sMessError.push('Hãy nhập giá trị phê duyệt dòng ' + sGiaTri.substring(0, sGiaTri.length - 2) + '.');
        }
    }
    ...
    while (iID_ParentID != "") {
        ...
        for (var p = 0; p < Bang_arrGiaTri.length; p++) {
            if (Bang_arrGiaTri[p][Bang_arrCSMaCot["iID_ParentID"]] == iID_ParentID && !Bang_arrHangDaXoa[p]) {
                sum += parseFloat(Bang_arrGiaTri[p][Bang_arrCSMaCot["fGiaTri"]]);
            }
        ...
        Bang_GanGiaTriThatChoO(indexParent, Bang_arrCSMaCot["fGiaTri"], sum);
    }
}
    ...
    if (BangDuLieu_DuocSuaChiTiet && h != null && bHasChild == 'False') {
        ...
        if (!Bang_arrHangDaXoa[h]) {
            ...
            if (countRow == 0) {
                ...
                Bang_arrEdit[numParent][Bang_arrCSMaCot["fGiaTri"]] = true;
            }
            ...
            if (countRowDelete == 0) {
                ...
                for (var p = 0; p < Bang_arrGiaTri.length; p++) {
                    if (Bang_arrGiaTri[p][Bang_arrCSMaCot["ID"]] == iID_ParentID) {
                        ...
                        Bang_arrEdit[p][Bang_arrCSMaCot["fGiaTri"]] = false;
                    }
                }
            }
        }
...
function BangDuLieu_onKeypress_F9(h, c) {
    $("#sTenNhiemVuChi, #sTenPhongBan").val("");
